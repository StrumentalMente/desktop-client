...
class Dialog {
	constructor(id = undefined, win = window, doc = document) {
		...
		this._Mousetrap = parent.require("mousetrap")(doc);
	...
	_setDialog(set, config) {
		...
		this._Mousetrap.bind("esc", () => {
			self.close();
			
			// L'istruzione seguente non aumenta di molto la complessità
			// dell'operazione: benché sembri essere lineare il vettore non
			// conterrà mai un numero elevato di elementi (da delle fasi di test
			// si è notato che non vengono quasi mai superati i tre elementi su
			// un pc altamente performante, e il loro numero è inversamente
			// proporzionale alla velocità della macchina su cui l'applicazione
			// è avviata).
			Array.from(document.querySelectorAll(".dialog-box, .dialog-box-overlay")).forEach(el => {
