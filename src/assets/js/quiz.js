...
* @param {String} fail_return_link Indica in quale pagina si deve ritornare al completamento dei quiz se non lo si è superato.
* @param {String} success_return_link Indica in quale pagina si deve ritornare al completamento dei quiz se lo si è superato.
*/
function quizVerify(fail_return_link, success_return_link) {
	answersCheck();
	if (document.getElementsByName("chord")[0] != undefined)
		verify_and_store();

	let passed = sessionStorage.getItem('score') > (sessionStorage.getItem('score_max') / 2);
	showQuizDialog(quiz_id, sessionStorage.getItem('score'), sessionStorage.getItem('score_max'), (passed ? success_return_link : fail_return_link));
}
...
/**
 * Verifica le risposte selezionate dall'utente con quelle corrette. In caso di
 * risposta corretta aumenta di 1 il punteggio ottenuto dall'utente.
 */
...
/**
 * Apre una pagina basandosi sul risultato del quiz in cui la presente funzione
 * viene richiamata.
 * @param {String} failedPage La pagina da aprire se il quiz non è stato
 * superato
 * @param {String} passedPage La pagina da aprire se il quiz è stato superato
 */
function quizSelectNextPage(failedPage, passedPage) {
	let passed = sessionStorage.getItem('score') > (sessionStorage.getItem('score_max') / 2);
	let link = passed ? passedPage : failedPage;
	window.location.href = `../../index.html?started=true&topic=${link}&to-home=true`;
}

/* 
 * La presente funzione è richiamata solo nelle pagine dei quiz.
 */
(function () {
	...
	Mousetrap.bind("enter", () => {
		if (!Dialog.isOpen()) {
			...
			let checkBtn = document.getElementById("end");
			let exitBtn = document.getElementById("exit");

			let toBeClicked = (nextBtn.style.display != "none") ? nextBtn : (checkBtn.style.display != "none") ? checkBtn : exitBtn;
